"""
game_state.py — центральное состояние кампании и игровая логика.

Реализует:
- Класс SiegeInfo: данные одной осады (цель, атакующие, ходы до капитуляции, снабжение, катапульты).
- Класс GameState: этап, год, ход, золото; владельцы и гарнизоны крепостей; полевая армия; осады; постройки и прогресс; наместники; полководцы; законы; отношения с AI; легитимность, недовольство, здоровье султана, уведомления.
- Методы владения и отношений: get_fortress_owner, get_relation_with, owned_fortresses.
- Экономика и модификаторы законов: _get_law_modifiers, учёт зданий при доходе/найме/обороне.
- Осада и штурм: can_besiege, can_assault, start_siege_from_field_army, assault_from_field_army, _capture_fortress (смена владельца, этап, легитимность, недовольство).
- Найм войск, строительство, наместники, полководец, перенос столицы, переименование.
- Ход: next_turn (доход, расходы, прогресс строительства, осады, AI, победа/поражение).
"""
...
class SiegeInfo:
    """
    Данные одной осады.
    target_fortress_id — осаждённая крепость;
    source_fortress_id — откуда осада (крепость или "field_army");
    attacker_troops — число атакующих;
    turns_remaining — ходов до капитуляции;
    defender_supplies — запасы защитников (при 0 быстрее капитуляция);
    catapults — число катапульт (ускоряет осаду).
    """
...
class GameState:
    """
    Состояние кампании на один момент времени.
    Содержит: этап (beylik/sultanate/empire), год, ход, золото;
    владельцы крепостей и гарнизоны (в т.ч. по типам войск);
    полевая армия и её полководец; осады; постройки и очередь строительства;
    наместники; принятые законы; отношения с AI и торговые партнёры;
    легитимность, недовольство в крепостях, здоровье султана, уведомления, предложения AI.
    """
    ...
    def next_turn(self) -> list[str]:
        """
        Выполнить один игровой ход: год/ход, расходы на армию, доход (крепости, зима, дань, торговля),
        прогресс строительства, обработка осад (снабжение, капитуляция), снижение недовольства,
        здоровье султана раз в 50 ходов, ходы всех AI. Возвращает список id захваченных крепостей.
        """
        self.turn += 1
        if self.turn % 3 == 0:
            self.year += 1
